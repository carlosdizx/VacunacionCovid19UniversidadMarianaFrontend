    ...
    this.service.getGraficaCuatro().subscribe(respuesta =>{
      ...
      this.graficaCuatro.forEach(datos => {
          if (datos.tipo==='Estudiante'){
            this.infoUnoE.push({data:[datos.cantidad],label: datos.tipo+'-'+datos.estado,backgroundColor:'RED',hoverBackgroundColor:'RED'})
          }
          else if (datos.tipo==='Docente'){
            this.infoUnoD.push({data:[datos.cantidad],label: datos.tipo+'-'+datos.estado,backgroundColor:'BLUE',hoverBackgroundColor:'BLUE'})
          }
          else if (datos.tipo==='Directivo'){
            this.infoUnoDI.push({data:[datos.cantidad],label: datos.tipo+'-'+datos.estado,backgroundColor:'GREEN',hoverBackgroundColor:'GREEN'})
          }
          else {
            this.infoUnoA.push({data:[datos.cantidad],label: datos.tipo+'-'+datos.estado,backgroundColor:'YELLOW',hoverBackgroundColor:'YELLOW'})
          }
        }
    )
      ...
      this.barChartData.push({data:[0],label:'',backgroundColor:'DARK',hoverBackgroundColor:'DARK'})
    });
  }
}
